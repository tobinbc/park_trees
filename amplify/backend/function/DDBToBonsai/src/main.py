...
secretsClient = Session().client(service_name='secretsmanager',region_name='eu-west-2')
get_secret_value_response = secretsClient.get_secret_value(
            SecretId='arn:aws:secretsmanager:eu-west-2::secret:medium-bonsai-BHiuhN'
        )
...
# Low-level POST data
def post_data_to_es(payload, path):
    '''Post data to ES endpoint'''
    response = requests.post(url = ES_ENDPOINT + path,data = payload, headers={'content-type':'application/json'})
    if response.status_code >= 200 and response.status_code <= 299:
        return response._content
    else:
        raise ES_Exception(response.status_code, response._content)
...
def post_to_es(payload):
    ...
    while retries < ES_MAX_RETRIES:
        ...
        try:
            es_ret_str = post_data_to_es( payload, '/_bulk')
